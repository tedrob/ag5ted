...
export class TickerDirective implements OnInit, OnDestroy {
  ...
  idle: boolean;
    ...
    @Input() speed: number;                  // milliseconds between ticks
    ...
    moveLeft(): void {
        const resetMargin = ( this.textWidth + this.paddingRight ) * -2 ;
    ...
    getTextWidth(): number {
        const t = this.r.createElement( document.getElementById('ghost'), 'div' );
